...
function OctaveSpectrogram(path,separador,leerDesde,canales,fs,a,b,canal,lin,tfr,wlen,dynr)
if(nargin!=12)
...
if (lin == 1)
  c_sgram = sgram(X(a:b,canal), fs, 'tfr', tfr, 'wlen', round(wlen/1000*fs), 'lin','dynrange', dynr, 'nodisplay');
else
  c_sgram = sgram(X(a:b,canal), fs, 'tfr', tfr, 'wlen', round(wlen/1000*fs), 'dynrange', dynr, 'nodisplay');
endif
