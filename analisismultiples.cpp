...
void AnalisisMultiples::setDatos(DelSignal *s)
{
    listSignals[btnSender] = new DelSignal();
    ...
    funcionesGraficas *FGr = new funcionesGraficas();
    FGr->cargaGrafica(listCheckBox[btnSender],listGraph[btnSender],listcBox[btnSender],canalActual,listSignals[btnSender]);
...
void AnalisisMultiples::on_actionReplot_triggered()
{
    if(listCheckBox[indCBox]->isEnabled())
    {
        ...
        FGr->cargaGrafica(listCheckBox[indCBox],listGraph[indCBox],listcBox[indCBox],canalActual,listSignals[indCBox]);
        return;
    }else{
...
void AnalisisMultiples::on_actionSpectrogram_triggered()
{
    ...
    if(octaveP->getState())
    {
        ...
        {            
            ...
            QString cmd = "OctaveSpectrogram('"+listSignals[indCBox]->getPath()+"','"+listSignals[indCBox]->getSeparador()+"','"+listSignals[indCBox]->getLeerDesde()+"','"+listSignals[indCBox]->getCanales()+"','"+listSignals[indCBox]->getFs()+"','"+QString::number(min)+"','"+QString::number(max)+"','"+QString::number(canalActual)+"','"+parametros[0]+"','"+parametros[1]+"','"+parametros[2]+"');\n";
...
void AnalisisMultiples::on_actionGabor_Transform_triggered()
{
    ...
    if(octaveP->getState())
    {
        ...
        {            
            ...
            QString cmd = "OctaveGabor('"+listSignals[indCBox]->getPath()+"','"+listSignals[indCBox]->getSeparador()+"','"+listSignals[indCBox]->getLeerDesde()+"','"+listSignals[indCBox]->getCanales()+"','"+listSignals[indCBox]->getFs()+"','"+QString::number(min)+"','"+QString::number(max)+"','"+QString::number(canalActual)+"','"+parametros[0]+"','"+parametros[1]+"','"+parametros[2]+"','"+parametros[3]+"');\n";
...
{
    QMessageBox::information(this, "Error", "Metodo aun no implementado", QMessageBox::Ok);
}
...
void AnalisisMultiples::chargeData(){
    ...
    funcionesGraficas *fgr = new funcionesGraficas();
    fgr->setParent(this->centralWidget());
...
//    // Se discrimina para saber que se hara con los datos recibidos y se llama a la funcion correspondiente.
    switch (analisis) {
    case 0:
        fgr->graficarFourier(listSignals[indCBox]->resultSignal, 0, listSignals[indCBox]->getFs().toDouble()*500, ConfigGlobales->getParametrosFourier());
        break;
    case 1:
        //graficarFourier(sigActual->resultSignal, graffico, 0, signalPrincipal->getFs().toDouble()*500);
        break;
    case 2:
        fgr->graficarGabor(listSignals[indCBox]->resultSignal, a, b, listSignals[indCBox]->getFs().toDouble());
        break;
    case 3:
        //graficarWavelets();
        break;
    case 4:
        fgr->graficarSpectrogram(listSignals[indCBox]->resultSignal, a, b, listSignals[indCBox]->getFs().toDouble() );
        break;
    case 5:        
        fgr->graficarHistograma(listSignals[indCBox]->resultSignal, "hist_normal");
        break;
    case 6:
        fgr->graficarHistograma(listSignals[indCBox]->resultSignal, "hist_fourier");
        break;
    case 7:
        fgr->graficarMovingRms(listSignals[indCBox]->resultSignal.at(1),listSignals[indCBox]->readSignal.at(0), min, max);
        break;
    case 8:
        parametros = ConfigGlobales->getParametrosMeanFrequency();
        fgr->graficarMeanFrequency(listSignals[indCBox]->resultSignal.at(1),listSignals[indCBox]->getFs().toDouble(), min, max, parametros[2]);
        break;
    default:
        break;
    }
